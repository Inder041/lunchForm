...
function formSubmit(){
    ...
    return true; // uncomment this line to bypass the client-side validations
    ...
    */
    ...
    if(errors.trim() != ''){ // trim is the function that trims any empty spaces from front or back
        ...
        return false; // stop the form from submitting to the server.
    }
    else{
        // If no errors found
        return true; // we do not need the rest of the code here as we will be processing on the server side.

        // The code below is client side processing.
    ...
    return false; // now that we are programming on the server side, you cannot always return false. You will have to either return true, if there are no errors or return false, if there are errors.
}